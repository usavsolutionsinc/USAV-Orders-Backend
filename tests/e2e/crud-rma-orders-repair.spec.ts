...
test.describe('orders tracking sub-resource', () => {
  ...
  test('set → edit → delete primary tracking', async ({ request }) => {
    ...
    // The record GET (getOrderById = SELECT *) exposes shipment_id but NOT the
    // joined tracking string, so we assert on shipment linkage: null → linked
    // → null. Carrier detection + the STN write are exercised by the route.
    const before = (await (await request.get(`/api/orders/${orderId}`)).json()).order;
    const originalShipmentId: number | null = before?.shipment_id ? Number(before.shipment_id) : null;

    // SET primary tracking → a shipment gets linked.
    ...
    const linkedShipmentId = Number(afterSet?.shipment_id);
    expect(Number.isFinite(linkedShipmentId) && linkedShipmentId > 0).toBe(true);

    // EDIT primary tracking → updated in place, shipment stays linked.
    ...
    expect(Number(afterEdit?.shipment_id) > 0).toBe(true);

    // DELETE unlinks the shipment from the order (?shipment_id=).
    const editedShipmentId = Number(afterEdit?.shipment_id) || linkedShipmentId;
    ...
    expect(afterDel?.shipment_id ?? null).toBeNull();

    // Best-effort restore of the original shipment link.
    if (originalShipmentId) {
      await request.patch(`/api/orders/${orderId}/tracking`, {
        data: { setPrimaryShipmentId: originalShipmentId },
      });
    }
  });
