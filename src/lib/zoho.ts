...
export async function listBillsForPurchaseOrder(
  ...
  for (;;) {
    ...
    page += 1;
    if (page > 25) break; /* hard cap — 5,000 bills scanned */
  }
...
export interface ZohoPurchaseReceiveLine {
  line_item_id: string;
  /** Quantity to record on this receive (POST body field is `quantity`). */
...
export interface ZohoPurchaseOrder {
  ...
  /** Bills raised for this PO (GET detail). Used for receive-after-bill flows. */
...
/**
 * Full `line_items` array for PUT /purchaseorders/{id} with per-line `description`
 * updates (Zoho expects the entire array). `lineItemIdToSerialNote` values are
 * merged into each line's description (e.g. "SN: ABC123").
 */
...
/**
 * Create-response payloads use `purchasereceive` or `purchase_receive` depending on API surface.
 */
...
export async function createPurchaseReceive(params: {
  ...
  warehouseId?: string;
  date?: string;
  /** Zoho display reference; auto-generated when omitted. */
  receiveNumber?: string;
  lineItems: ZohoPurchaseReceiveLine[];
  /** Zoho internal bill id for billed PO receives; optional if PO detail includes `bills`. */
  billId?: string;
  /**
   * Matches `bill_number` on GET PO `bills[]` when several bills exist.
   * (Not the PO# — use the Bill document number Zoho shows on the vendor bill.)
   */
  ...
  try {
    ...
    if (!billIdHint) {
      // PO detail omits `bills[]` on some Zoho org plans even when the PO is
      // billed (confirmed against PO 5623409000002369182). Fall back to the
      // /bills list endpoint filtered by purchaseorder_id.
