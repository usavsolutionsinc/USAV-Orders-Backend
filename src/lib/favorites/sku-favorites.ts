...
export async function listFavoriteSkus(workspaceKey: FavoriteWorkspaceKey, includeInactive = false, orgId?: OrgId): Promise<FavoriteSkuRecord[]> {
  ...
  if (orgId) {
    const result = await withTenantConnection(orgId, (client) =>
      client.query(
        ...
        `,
        [workspaceKey, includeInactive, orgId],
      ),
    );
...
export async function createFavoriteSku(input: CreateFavoriteSkuInput, orgId?: OrgId): Promise<FavoriteSkuRecord> {
  ...
  if (orgId) {
    // withTenantConnection owns BEGIN/COMMIT/ROLLBACK + SET LOCAL app.current_org.
    return withTenantConnection(orgId, async (client) => {
      const favoriteResult = await client.query(
        `
          ...
          DO UPDATE SET
            ...
            updated_at = NOW()
          WHERE favorite_skus.organization_id = $10
          RETURNING id
        `,
        [
      ...
      if (favoriteResult.rows.length === 0) {
        // sku_normalized is a GLOBAL unique key; a conflict against another
        // org's row hits the ON CONFLICT but is excluded by the org guard, so
        // no row is returned. Surface that rather than mis-attributing.
        throw new Error('SKU is already favorited by another organization');
      }
      ...
      await client.query(
        `
          ...
          DO UPDATE SET
            ...
            updated_at = NOW()
          WHERE favorite_sku_workspaces.organization_id = $5
        `,
        [
          favoriteId,
          input.workspaceKey,
...
export async function updateFavoriteSku(input: UpdateFavoriteSkuInput, orgId?: OrgId): Promise<FavoriteSkuRecord | null> {
  assertWorkspaceKey(input.workspaceKey);
  ...
  if (orgId) {
    ...
    return withTenantConnection(orgId, async (client) => {
      ...
      // Org-ownership 404: not found OR owned by another org → null (caller 404s).
      ...
      if (input.sortOrder !== undefined || input.isActive !== undefined) {
        await client.query(
          ...
          `,
          [input.id, input.workspaceKey, input.sortOrder ?? null, input.isActive ?? null, orgId],
        );
      }
      ...
      const recordResult = await client.query(
        `
          ...
          WHERE f.id = $1
            ...
            AND f.organization_id = $3
        `,
        [input.id, input.workspaceKey, orgId],
      );
...
export async function deleteFavoriteSku(id: number, workspaceKey: FavoriteWorkspaceKey, orgId?: OrgId): Promise<boolean> {
  assertWorkspaceKey(workspaceKey);
  ...
  if (orgId) {
    ...
    return withTenantConnection(orgId, async (client) => {
      const deleteResult = await client.query(
        ...
        [id, workspaceKey, orgId],
      );

      // Org-ownership: nothing deleted (absent or another org's row) → false.
      if ((deleteResult.rowCount ?? 0) === 0) {
        return false;
      }

      const remainingResult = await client.query(
        `SELECT 1 FROM favorite_sku_workspaces WHERE favorite_id = $1 AND organization_id = $2 LIMIT 1`,
        [id, orgId],
      );
      if (remainingResult.rows.length === 0) {
        await client.query(
          `DELETE FROM favorite_skus WHERE id = $1 AND organization_id = $2`,
          [id, orgId],
        );
      }
