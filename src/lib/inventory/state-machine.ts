...
const TRANSITIONS: Readonly<Record<SerialState, ReadonlySet<SerialState>>> = {
  ...
  GRADED:      new Set<SerialState>(['STOCKED', 'SCRAPPED', 'IN_TEST' /* test reset: clear a wrong PASS verdict */, 'IN_REPAIR' /* graded unit found defective → repair */, 'ALLOCATED' /* paired straight to an order from graded */, 'TESTED' /* recordTestVerdict PASS on an already-graded unit (TESTED/GRADED are sibling test-result states) */, 'SHIPPED' /* FBA direct-ship: a sellable graded unit staged to an FBA shipment ships straight to Amazon */, 'PICKED' /* force-pick override (pick/scan override_mismatch: graded sellable unit picked without an allocation) */]),
  TESTED:      new Set<SerialState>(['STOCKED', 'IN_REPAIR', 'SCRAPPED', 'ALLOCATED' /* paired straight to an order from tested */, 'IN_TEST' /* recordTestVerdict TEST_AGAIN: re-test a passed unit */, 'SHIPPED' /* FBA direct-ship (sellable tested unit) */, 'PICKED' /* force-pick override (pick/scan override_mismatch: operator picks a sellable unit with no open allocation) */]),
  STOCKED:     new Set<SerialState>(['ALLOCATED', 'RETURNED', 'SCRAPPED', 'RECEIVED', 'TESTED', 'GRADED' /* un-putaway: back to whichever pre-stock state the unit came from (mark-received stocks straight from RECEIVED; testing via TESTED/GRADED) */, 'IN_REPAIR' /* pull stock back for repair */, 'SHIPPED' /* FBA direct-ship from stock (FBA link-unit normally allocates first, but cover a stock unit that reaches ship directly) */, 'PICKED' /* force-pick override (pick/scan override_mismatch: stock unit picked without an allocation) */]),
