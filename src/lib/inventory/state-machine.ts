...
const TRANSITIONS: Readonly<Record<SerialState, ReadonlySet<SerialState>>> = {
  ...
  REPAIR_DONE: new Set<SerialState>(['IN_TEST', 'GRADED', 'IN_REPAIR' /* re-open repair (more work / failed re-test) */]),
  IN_TEST:     new Set<SerialState>(['GRADED', 'IN_REPAIR', 'SCRAPPED', 'RECEIVED' /* test reset: un-start back to received */]),
  GRADED:      new Set<SerialState>(['STOCKED', 'SCRAPPED', 'IN_TEST' /* test reset: clear a wrong PASS verdict */, 'IN_REPAIR' /* graded unit found defective → repair */, 'ALLOCATED' /* paired straight to an order from graded */]),
  TESTED:      new Set<SerialState>(['STOCKED', 'IN_REPAIR', 'SCRAPPED', 'ALLOCATED' /* paired straight to an order from tested */]),
