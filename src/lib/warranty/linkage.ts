...
async function loadClaimCore(client: PoolClient, claimId: number): Promise<ClaimCore | null> {
...
export async function issueRmaForClaim(
  ...
  try {
    ...
    const claim = await loadClaimCore(client, claimId);
    if (!claim) {
    ...
    const recheck = await loadClaimCore(client, claimId);
    if (!recheck) {
      await client.query('ROLLBACK');
      return { ok: false, status: 404, error: 'claim not found' };
    }
    ...
    await client.query(`UPDATE warranty_claims SET rma_id = $2, updated_at = NOW() WHERE id = $1`, [
      claimId,
      rma.id,
    ]);
    await insertEvent(
