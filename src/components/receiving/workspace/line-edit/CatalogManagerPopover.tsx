...
/**
 * CRUD manager for the org platform / type catalog. Opened from the pencil next
 * to the Platform or Type dropdown in {@link LabelEditPopover}. Lists the org's
 * entries (active + hidden) with inline rename, reorder, hide/restore, add, and
 * delete — backed by /api/catalog/platforms|types. Same RightPaneOverlay shell
 * as the label editor / ReceivingClaimModal.
 *
 * Built-in (`is_system`) rows are protected: badged "Default", slug immutable,
 * and hide-only (a soft-delete the operator can Restore) rather than removed —
 * so an org can rename / retone / reorder / hide a default but never destroy
 * it. Custom rows are fully removable (also soft, so still restorable).
 *
 * When the catalog has no DB rows yet (migration unapplied) it shows the
 * built-in defaults read-only with a one-line notice, so it never looks broken.
 */
...
export function CatalogManagerPopover({
  ...
  const platformQ = useQuery({ ...platformsQuery({ includeInactive: true }), enabled: open && kind === 'platform' });
  const typeQ = useQuery({ ...typesQuery({ includeInactive: true }), enabled: open && kind === 'type' });
  ...
  // Fallback display when the catalog has no rows yet (migration unapplied).
  ...
  // Swap an active row's sort with its active neighbour in the chosen direction.
  ...
  return (
    ...
    >
        ...
        {!editable ? (
          <>
            <div className="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-micro font-semibold text-amber-800">
              Showing built-in defaults. Apply migration <code>2026-06-13g</code> to add or edit
              your own.
            </div>
      ...
      {/* Footer — add */}
      <div className="flex shrink-0 items-center gap-2 border-t border-gray-100 bg-gray-50 px-5 py-3">
        <input
          value={adding}
