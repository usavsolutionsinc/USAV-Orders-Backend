...
import { Package, Loader2, Check } from '@/components/Icons';
...
function EcwidPairingTable({ basePath }: { basePath: string }) {
...
function CatalogTable({ basePath }: { basePath: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
...
/**
 * Main-pane content for the manuals workspace. Reads `mode` from the URL,
 * forks between the Ecwid pairing flow and the SKU catalog table, and writes
 * row-selection state back to the URL under `basePath` (which is `/products`
 * once `/manuals` is folded into the Products workspace).
 */
export function ManualsTables({ basePath }: { basePath: string }) {
  const searchParams = useSearchParams();
  const mode = (searchParams.get('mode') || 'all') as Mode;
  if (mode === 'pairing') return <EcwidPairingTable basePath={basePath} />;
  return <CatalogTable basePath={basePath} />;
}

export default ManualsTables;
