...
export function FolderButton({
  ...
  return (
    ...
    <div
      className={`group relative flex w-full items-center gap-3 rounded-2xl border bg-surface-card px-3 py-2.5 text-left shadow-sm transition-all hover:-translate-y-px hover:border-indigo-200 hover:bg-indigo-50/30 hover:shadow-md active:translate-y-0 active:shadow-sm ${
        dragOver ? 'border-indigo-400 bg-indigo-50 ring-2 ring-indigo-300' : 'border-border-soft'
      }`}
    ...
    >
      ...
      <HoverTooltip label="Rename or move folder" asChild>
        ...
        <button
          ...
          className="relative inline-flex h-7 w-7 items-center justify-center rounded-lg text-text-faint opacity-0 transition-opacity hover:bg-surface-sunken hover:text-text-muted group-hover:opacity-100 focus:opacity-100"
          aria-label={`Rename or move ${node.name}`}
        >
          <Pencil className="h-3.5 w-3.5" />
        </button>
      </HoverTooltip>
      <ChevronRightTiny className="pointer-events-none relative h-3.5 w-3.5 shrink-0 text-text-faint transition-transform group-hover:translate-x-0.5 group-hover:text-indigo-400" />
    </div>
  );
}
