...
export function DashboardShippedSearchHandoffCard({
  ...
  return (
    <div className="rounded-2xl border border-border-accent bg-surface-accent/70 px-4 py-3">
      <div className="space-y-2">
        <Button
          ...
          className={`w-full justify-center rounded-xl border border-border-accent bg-surface-card px-3 py-2 ring-0 ${sectionLabel} text-text-accent hover:bg-surface-accent/80 disabled:cursor-not-allowed disabled:border-border-accent disabled:bg-surface-accent disabled:text-text-accent`}
        >
          Open Shipped
        </Button>

        <div className="min-w-0">
          <p className={`${sectionLabel} text-text-accent`}>
            Shipped Matches
          </p>
          <p className="mt-1 text-caption font-semibold text-text-accent">
            {isBusy ? 'Checking shipped results...' : `${shippedCount} found in shipped`}
          </p>
          <p className="mt-1 text-micro font-semibold leading-relaxed text-text-accent/80">
