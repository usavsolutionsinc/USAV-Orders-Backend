  ...
  return (
    <div className="mt-3 border-t border-gray-100 pt-3">
      <div className="mb-2 flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-micro font-semibold text-slate-500">{research.model}</span>
      </div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {candidate?.url ? (
                    <HoverTooltip label="Open listing" asChild>
                      <a href={candidate.url} target="_blank" rel="noreferrer" aria-label="Open listing" className="rounded-md p-1.5 text-gray-400 hover:bg-white hover:text-blue-600">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </a>
                    </HoverTooltip>
                  ) : null}
                  {candidate ? (
                    <Button variant="ghost" size="sm" onClick={() => onSave(candidate)} disabled={saving} className="text-emerald-700 hover:bg-white hover:text-emerald-700">
                      Save
                    </Button>
                  ) : null}
                </div>
...
function Score({ label, value }: { label: string; value: number }) {
  return (
    <div className="w-11 text-right">
      ...
      <p className="text-eyebrow font-semibold uppercase text-gray-400">{label}</p>
    </div>
  );
}
