...
import { Search, AlertCircle, Star, Clock, Link2, Sparkles, ExternalLink } from '@/components/Icons';
...
interface SourcingResearchResponse {
  query: string;
  total: number;
    ...
    <li className="rounded-xl border border-gray-200 bg-white p-3">
      ...
      {researchResult ? (
        <ResearchPanel
          ...
          candidates={researchResult.results}
          onSave={(candidate) => save.mutate(candidate)}
      ...
      <ul className="space-y-2">
        {rows.map((a) => (
          <QueueAlertRow
            key={a.id}
            alert={a}
            patchStatus={(status) => patch.mutate({ id: a.id, status })}
            setTarget={setTarget}
            close={close}
          />
        ))}
  ...
  const research = useMutation({
    mutationFn: () =>
      jsonFetch('/api/sourcing/research', {
        ...
        body: JSON.stringify({
          query,
          skuId: alert.sku_id ?? undefined,
          sourcingAlertId: alert.id,
          maxPriceCents: alert.replenish_target_cents ?? undefined,
          limit: 12,
        }),
      }) as Promise<SourcingResearchResponse>,
  ...
  return (
    <div className="mt-3 border-t border-gray-100 pt-3">
            ...
            return (
              <li key={`${ranked.externalId ?? ranked.title}-${index}`} className="grid grid-cols-[1fr_auto] gap-3 rounded-lg bg-gray-50 px-3 py-2">
                <div className="min-w-0">
                  ...
                  {ranked.riskFlags.length > 0 ? (
                    <p className="mt-1 truncate text-[11px] font-medium text-amber-700">{ranked.riskFlags.join(' · ')}</p>
                  ) : null}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  {candidate ? (
                    <button
                      ...
                      disabled={saving}
                      className="rounded-md px-2 py-1 text-caption font-semibold text-emerald-700 hover:bg-white disabled:opacity-50"
                    >
                      Save
                    </button>
                  ) : null}
      ...
      {research.cautions.length > 0 ? (
        <p className="mt-2 text-caption text-amber-700">{research.cautions.join(' · ')}</p>
      ) : null}
    </div>
  );
}
