    ...
    >
      <div className="space-y-4 px-1">
        ...
        {stations.length === 0 && identifiers.length === 0 && (
          <p className="px-2 py-6 text-center text-xs text-text-faint">No matches.</p>
        )}
      </div>
    </AdminSidebarShell>
  );
...
function RowShell({
  ...
  return (
    <li className={`overflow-hidden rounded-lg border ${open ? 'border-blue-300 bg-blue-50/40' : 'border-border-soft bg-surface-card'}`}>
      {/* ds-raw-button: full-width left-aligned accordion header (multi-line title/subtitle + chevron) */}
      <button type="button" onClick={onToggle} className="flex w-full items-center gap-2 px-2.5 py-2 text-left hover:bg-surface-hover">
        <Dot color={color} />
...
function IdentifierRow({ id, open, onToggle, onJump }: { id: OpsIdentifier; open: boolean; onToggle: () => void; onJump: (k: string) => void }) {
  return (
    <RowShell color="#0ea5e9" title={id.label} subtitle={id.example} open={open} onToggle={onToggle}>
      <p className="text-text-muted">{id.blurb}</p>
