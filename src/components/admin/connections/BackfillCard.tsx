...
export function BackfillCard({ embedded = false }: { embedded?: boolean }) {
  ...
  const statusInitial = shouldReduceMotion ? { opacity: 0 } : framerPresence.statusMessage.initial;
  const statusAnimate = shouldReduceMotion ? { opacity: 1 } : framerPresence.statusMessage.animate;
    ...
    <div className={embedded ? 'space-y-4' : 'space-y-4 border border-gray-200 bg-white p-5'}>
      <div className={`flex items-center justify-between gap-3 ${embedded ? 'border-b border-gray-200 pb-3' : ''}`}>
        <div>
          ...
          <p className="text-eyebrow font-bold text-gray-500 mt-1">Fill missing order fields from marketplace APIs without overwriting existing data.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all text-micro font-black uppercase tracking-widest text-white shadow-sm disabled:opacity-50"
          >
      ...
      {ebayBackfillMutation.isSuccess && (
        <motion.div initial={statusInitial} animate={statusAnimate} className="border-l-2 border-l-green-500 p-4 bg-green-50/70">
      ...
      {ebayBackfillMutation.isError && (
        <motion.div initial={statusInitial} animate={statusAnimate} className="border-l-2 border-l-red-500 p-4 bg-red-50/70">
          <div className="text-micro font-black text-red-700 uppercase tracking-widest">
