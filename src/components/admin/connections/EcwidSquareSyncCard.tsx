    ...
    <div className={embedded ? 'space-y-4' : 'space-y-4 border border-gray-200 bg-white p-5'}>
      <div className={`flex items-center justify-between gap-3 ${embedded ? 'border-b border-gray-200 pb-3' : ''}`}>
        <div>
          ...
          <p className="text-eyebrow font-bold text-gray-500 mt-1">Preview or run the one-way sync for enabled Ecwid products.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => syncMutation.mutate({ dryRun: true })}
            disabled={syncMutation.isPending}
            className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all text-micro font-black uppercase tracking-widest text-gray-700 disabled:opacity-50"
          >
            Preview
          </button>
          <button
      ...
      {syncMutation.isSuccess && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="border-l-2 border-l-green-500 p-4 bg-green-50/70 space-y-2">
          ...
          {counts && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-eyebrow font-bold text-green-800 uppercase tracking-wide">
          ...
          {typeof syncMutation.data?.batchSizeUsed === 'number' && (
            <div className="text-eyebrow font-bold text-green-800 uppercase tracking-wide">Batch size: {syncMutation.data.batchSizeUsed}</div>
          )}
        </motion.div>
      )}
