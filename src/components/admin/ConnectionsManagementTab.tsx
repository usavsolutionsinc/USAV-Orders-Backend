...
export function ConnectionsManagementTab() {
  ...
  const [zohoStatus, setZohoStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);
  const [zohoPurchaseReceiveId, setZohoPurchaseReceiveId] = useState('');
  ...
  const zohoRefreshMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || data?.message || `Zoho refresh failed (HTTP ${res.status})`);
      }
      return data;
    },
  });
  ...
  const zohoSyncMutation = useMutation({
    mutationFn: async () => {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || data?.message || `Zoho sync failed (HTTP ${res.status})`);
      }
      return data;
    },
  });
  ...
  const zohoImportOneMutation = useMutation({
    mutationFn: async (purchaseReceiveId: string) => {
      const res = await fetch('/api/zoho/purchase-receives/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ purchase_receive_id: purchaseReceiveId }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data?.success) {
        throw new Error(data?.error || data?.message || `Zoho import failed (HTTP ${res.status})`);
      }
      return data;
    },
  });
  ...
  const handleZohoRefresh = async () => {
    setZohoStatus(null);
    try {
      const data = await zohoRefreshMutation.mutateAsync();
      setZohoStatus({
        type: 'success',
        message: data?.message || 'Zoho token refresh completed successfully.',
      });
    } catch (error: any) {
      setZohoStatus({
        type: 'error',
        message: error?.message || 'Zoho token refresh failed',
      });
    }
  };
  ...
  const handleZohoSync = async () => {
    ...
    try {
      ...
      setZohoStatus({
        type: 'error',
        message: error?.message || 'Zoho sync failed',
      });
    }
  };

  const handleZohoImportOne = async () => {
    const id = zohoPurchaseReceiveId.trim();
    if (!id) return;
    setZohoStatus(null);
    try {
      ...
      setZohoStatus({
        type: 'error',
        message: error?.message || 'Zoho single import failed',
      });
    }
  };
      ...
      <div className="space-y-4 p-5 bg-white rounded-3xl border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => void handleZohoRefresh()}
              disabled={zohoRefreshMutation.isPending || zohoSyncMutation.isPending || zohoImportOneMutation.isPending}
            ...
            <button
              ...
              disabled={zohoSyncMutation.isPending || zohoRefreshMutation.isPending || zohoImportOneMutation.isPending}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest text-white shadow-sm disabled:opacity-50"
            >
        ...
        <div className="flex items-center gap-2">
          <input
            value={zohoPurchaseReceiveId}
            onChange={(e) => setZohoPurchaseReceiveId(e.target.value)}
          ...
          />
          <button
            onClick={() => void handleZohoImportOne()}
            disabled={!zohoPurchaseReceiveId.trim() || zohoImportOneMutation.isPending || zohoSyncMutation.isPending}
