...
const SOURCE_PILL: Record<PermissionSource, { className: string; text: string }> = {
  role:          { className: 'bg-gray-100 text-gray-600 ring-gray-200',          text: 'Role' },
  ...
  'role-denies': { className: 'bg-gray-100 text-gray-500 ring-gray-200',          text: 'Role denies' },
};
...
export function PageAccessSwitch({ label, permission, enabled, source, theme, disabled, busy, onToggle }: PageAccessSwitchProps) {
  ...
  return (
    <li className={`flex items-center gap-3 px-4 py-2.5 transition ${disabled ? 'opacity-60' : 'hover:bg-gray-50/60'}`}>
