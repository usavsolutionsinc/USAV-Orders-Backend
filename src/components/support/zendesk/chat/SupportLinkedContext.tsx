  ...
  return (
    <div className="border-t border-gray-100 bg-gray-50/60 px-5 py-4">
      <div className="flex items-center gap-1.5">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Linked context</p>
      </div>

      {entity?.type ? (
        <div className="mt-2 inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5">
          <span className="text-[12px] font-bold text-gray-700">{ENTITY_LABELS[entity.type] ?? entity.type}</span>
          {entity.id ? <span className="text-[11px] font-semibold text-gray-400">#{entity.id}</span> : null}
        </div>
      ) : null}

      {photos.length ? (
        <div className="mt-3">
          <p className="mb-1.5 flex items-center gap-1 text-[11px] font-semibold text-gray-400">
          ...
          <div className="flex flex-wrap gap-2">
            {photos.map((p) => (
              <button
                ...
                className="block h-20 w-20 overflow-hidden rounded-lg ring-1 ring-inset ring-gray-200 transition hover:opacity-90 hover:ring-2 hover:ring-blue-300"
              >
