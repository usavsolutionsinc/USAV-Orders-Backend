...
import { AlertTriangle, Check, ChevronDown, ChevronRight, Loader2, Pencil, Plus, Trash2, X } from '@/components/Icons';
        ...
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-[11px] font-black uppercase tracking-[0.16em] text-gray-900">
            ...
            {!isLoading && favorites.length > 0 && (
              <span className="ml-1.5 tabular-nums text-gray-400">{favorites.length}</span>
            )}
          </h3>
          <div className="ml-auto flex shrink-0 items-center gap-1.5">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-600 transition-colors hover:border-gray-900 hover:text-gray-900"
              aria-label={isListOpen ? 'Collapse favorites' : 'Expand favorites'}
              aria-expanded={isListOpen}
              title={isListOpen ? 'Collapse' : 'Expand'}
            >
              <ChevronDown
                className={`h-3.5 w-3.5 transition-transform duration-200 ${isListOpen ? 'rotate-180' : ''}`}
              />
            </button>
          </div>
        </div>
                ...
                {favorites.map((favorite) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex min-h-[132px] flex-col overflow-hidden rounded-xl border-2 border-gray-200 bg-white"
                    >
                      <div className="flex flex-1 flex-col p-4 pb-3">
                        ...
                        <div className="mt-2 flex min-w-0 items-baseline gap-2">
                          ...
                          {favorite.sku && (
                            <span className="min-w-0 truncate text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400">
                              {favorite.sku}
                            </span>
                          )}
                        </div>
