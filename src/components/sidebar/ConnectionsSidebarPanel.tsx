    ...
    <div className="flex h-full flex-col overflow-hidden bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        <SidebarSection title="Orders" expanded={showOrders} onToggle={() => setShowOrders((v) => !v)}>
          <LineItem label="Run Full Integrity" detail="eBay + Ecwid exceptions + reconciliation" right={<ActionButton onClick={() => fullIntegrityMutation.mutate()} loading={fullIntegrityMutation.isPending} title="Run full integrity" tone="green" />} />
          <LineItem label="eBay Sync" detail="Tracking-match and reconciliation" right={<ActionButton onClick={() => ebaySyncMutation.mutate()} loading={ebaySyncMutation.isPending} title="Sync eBay" tone="blue" />} />
          <LineItem label="Ecwid Exceptions" detail="Sync tracking onto open exceptions" right={<ActionButton onClick={() => ecwidExceptionTrackingMutation.mutate()} loading={ecwidExceptionTrackingMutation.isPending} title="Sync Ecwid exceptions" tone="blue" />} />
          <LineItem label="Exceptions Sync" detail="Clear resolved exceptions" right={<ActionButton onClick={() => exceptionsSyncMutation.mutate()} loading={exceptionsSyncMutation.isPending} title="Sync exceptions" tone="green" />} />
          <LineItem label="ShipStation CSV" detail="Upload a local CSV export" right={<button type="button" onClick={() => shipStationFileInputRef.current?.click()} className="h-full w-12 border-l border-gray-200 text-[10px] font-black uppercase tracking-widest text-gray-700 hover:bg-gray-100">Up</button>} />
          {tokenAccounts.map((account) => {
            ...
            return (
              <LineItem
                ...
                detail={minutesLeft <= 0 ? 'Expired token' : `${minutesLeft} minutes remaining`}
        ...
        <SidebarSection title="Zoho" expanded={showZoho} onToggle={() => setShowZoho((v) => !v)}>
          ...
          <div className="border-b border-gray-200 bg-white px-4 py-3">
            ...
            <div className="mt-2 flex items-stretch gap-0 border border-gray-200">
              ...
              >
                {zohoImportOneMutation.isPending ? '...' : 'Go'}
              </button>
            </div>
          </div>
        </SidebarSection>
        ...
        <SidebarSection title="Backfill" expanded={showBackfill} onToggle={() => setShowBackfill((v) => !v)}>
          <LineItem label="eBay" detail="Backfill only empty order fields" right={<ActionButton onClick={() => ebayBackfillMutation.mutate()} loading={ebayBackfillMutation.isPending} title="Backfill eBay" tone="indigo" />} />
          <LineItem label="Ecwid" detail="Backfill only empty order fields" right={<ActionButton onClick={() => ecwidBackfillMutation.mutate()} loading={ecwidBackfillMutation.isPending} title="Backfill Ecwid" tone="indigo" />} />
        </SidebarSection>

        <SidebarSection title="Catalog" expanded={showCatalog} onToggle={() => setShowCatalog((v) => !v)}>
          <LineItem label="Ecwid -> Square Dry Run" detail="Preview enabled product sync" right={<ActionButton onClick={() => ecwidSquareSyncMutation.mutate({ dryRun: true })} loading={ecwidSquareSyncMutation.isPending && ecwidSquareSyncMutation.variables?.dryRun === true} title="Run Ecwid Square dry run" />} />
          <LineItem label="Ecwid -> Square Sync" detail="Run live catalog upserts" right={<ActionButton onClick={() => ecwidSquareSyncMutation.mutate({ dryRun: false })} loading={ecwidSquareSyncMutation.isPending && ecwidSquareSyncMutation.variables?.dryRun === false} title="Run Ecwid Square sync" tone="blue" />} />
        </SidebarSection>
          ...
          {(['USPS', 'UPS', 'FEDEX'] as const).map((carrier) => {
            ...
            return (
              <LineItem
                key={carrier}
                label={carrier}
                detail="Sync due carrier tracking updates"
