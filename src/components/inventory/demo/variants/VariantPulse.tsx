...
export function VariantPulse() {
    ...
    return (
        <div className="flex h-full w-full overflow-hidden bg-gray-50/30">
            {/* Sidebar: Global Pulse Feed */}
            <aside className="w-80 shrink-0 border-r border-gray-200 bg-white flex flex-col">
                <div className="p-4 border-b border-gray-200 space-y-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xs font-black uppercase tracking-widest text-gray-900">System Pulse</h2>
                        <span className="flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-black text-emerald-600 uppercase tracking-widest border border-emerald-100">
                    ...
                    <div className="relative">
                        ...
                        <input 
                            type="text" 
                            placeholder="Track Unit ID or Serial..." 
                ...
                <div className="flex-1 overflow-y-auto">
                    {MOCK_EVENTS.map(event => (
                        <button 
                            ...
                            className={cn(
                                "w-full p-4 text-left border-b border-gray-50 transition-all relative group",
                                selectedEventId === event.id ? "bg-blue-50/50 shadow-[inset_3px_0_0_0_#3b82f6]" : "hover:bg-gray-50"
                            )}
                        >
                            <div className="flex items-start gap-3">
                                <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shrink-0 shadow-sm border border-white/50", event.bg, event.color)}>
                                    <event.icon className="h-4 w-4" />
                                </div>
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center justify-between mb-0.5">
                                        <h3 className={cn("text-[11px] font-black uppercase tracking-tight truncate", selectedEventId === event.id ? "text-blue-700" : "text-gray-900")}>
                                            {event.title}
                                        </h3>
                                        <span className="text-[9px] font-bold text-gray-400 whitespace-nowrap ml-2">{event.time}</span>
                                    </div>
                                    <p className="text-[11px] text-gray-500 truncate mb-1">{event.details}</p>
                                    <div className="flex items-center gap-1.5">
                                        <div className="h-3.5 w-3.5 rounded-full bg-gray-200 flex items-center justify-center text-[7px] font-bold text-gray-600">
                                            {event.user.charAt(0)}
                                        </div>
                                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{event.user}</span>
                                    </div>
                                </div>
                            </div>
                        </button>
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="rounded-lg bg-gray-900 px-3 py-1 text-xs font-black text-white uppercase tracking-widest">UNIT #88129</span>
                        ...
                        <div className="col-span-7 space-y-1">
                            ...
                            <div className="relative pl-8 before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100">
                                {MOCK_EVENTS.map((event, i) => (
                                    <div key={event.id} className="relative pb-10 last:pb-0 group">
                                        {/* Dot */}
                                        <div className={cn(
                            ...
                            <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-xl shadow-gray-100 space-y-6">
                                ...
                                <div className="pt-4 border-t border-gray-50">
                                    <div className="flex items-center justify-between mb-4">
                                        <span className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Nearby Stock</span>
                                        <button className="text-[10px] font-black text-blue-600 hover:underline uppercase tracking-widest">See Map</button>
                                    </div>
                                    <div className="space-y-2">
                                        {[1, 2, 3].map(i => (
                                            <div key={i} className="flex items-center justify-between p-2 rounded-xl bg-gray-50 border border-gray-100">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[11px] font-bold text-gray-700 uppercase">A-01-0{i+2}</span>
                                                </div>
                                                <span className="text-[11px] font-black text-gray-900">4 units</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
