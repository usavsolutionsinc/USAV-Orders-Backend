...
import { useAddTrackingNav } from '@/components/unshipped/add-tracking-context';
...
/**
 * Rich "Add TRK#" popover for an Unshipped order: shows the order's identity,
 * an add-tracking input, and fill-missing SKU/item# fields with real DB linkage
 * (SKU resolves via /api/get-title-by-sku → stores sku + sku_catalog_id). Prev/
 * Next walk the needs-tracking worklist (via AddTrackingNavProvider) and a
 * "recently added" log shows throughput. Replaces the bare paste affordance.
 */
export function AddTrackingPopover({ record }: { record: ShippedOrder }) {
  ...
  // Re-init fields each time this order's popover opens (the instance persists
  // on the row, so a prior abandoned edit shouldn't linger).
  useEffect(() => {
    if (!open) return;
    setTracking('');
    setSku(initialSku);
  ...
  // Debounced SKU → title/catalog-id resolution (never string-joined; the API
  // resolves items→sku_catalog and returns the canonical sku_catalog_id).
  useEffect(() => {
  ...
  const handlePaste = async () => {
    try {
      ...
      /* clipboard blocked — operator can type */
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Advance to the next order needing tracking, or close at the end.
      ...
      <Popover.Portal>
        ...
        >
          ...
          <div className="mb-2 flex items-center justify-between gap-2">
            <span className={`${sectionLabel} text-blue-700`}>Add Tracking</span>
          ...
          <div className="mb-3 flex items-center gap-1.5">
            <input
              ...
              className="min-w-0 flex-1 rounded-xl border border-gray-200 bg-white px-3 py-2 font-mono text-caption text-gray-900 outline-none transition-all focus:border-blue-500"
            />
            <button
              type="button"
              onClick={handlePaste}
              title="Paste from clipboard"
              className="shrink-0 rounded-xl border border-gray-200 p-2 text-gray-500 transition-colors hover:bg-gray-50 hover:text-blue-600"
            >
              <Clipboard className="h-4 w-4" />
            </button>
          </div>

          {/* Fill missing — SKU + item# */}
          <label className="mb-1 block text-eyebrow font-black uppercase tracking-wider text-gray-500">SKU</label>
          <input
            value={sku}
            onChange={(e) => setSku(e.target.value)}
            placeholder="SKU"
            className="mb-1 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 font-mono text-caption text-gray-900 outline-none transition-all focus:border-blue-500"
          />
          ...
          <input
            ...
            className="mb-3 w-full rounded-xl border border-gray-200 bg-white px-3 py-2 font-mono text-caption text-gray-900 outline-none transition-all focus:border-blue-500"
          />
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={!canSave}
              className={`flex flex-1 items-center justify-center gap-2 rounded-xl bg-blue-600 py-2.5 text-white transition-all hover:bg-blue-700 active:scale-95 disabled:opacity-40 ${sectionLabel} !text-white`}
            >
