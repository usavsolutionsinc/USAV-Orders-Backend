  ...
  return (
    ...
    >
      ...
      <div className={`flex-shrink-0 w-10 h-8 rounded-lg flex flex-col items-center justify-center ${
        ...
        <span className={`text-[7px] font-black uppercase leading-none ${isActive ? 'text-indigo-500' : 'text-gray-400'}`}>
          {carrierShort}
        </span>
        <span className={`text-[9px] font-black font-mono leading-none mt-0.5 ${isActive ? 'text-indigo-700' : 'text-gray-600'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className={`text-[10px] font-black tabular-nums ${isActive ? 'text-indigo-700' : 'text-gray-700'}`}>
            #{box.receiving_id}
          </span>
          {receivedTime && (
            <span className="text-[9px] font-medium text-gray-400">{receivedTime}</span>
          )}
          {box.has_test_items && (
            <span className="text-[7px] font-black uppercase px-1 py-0.5 rounded bg-orange-100 text-orange-600">Test</span>
          )}
          {box.zoho_purchaseorder_id && (
            <span className="text-[7px] font-black uppercase px-1 py-0.5 rounded bg-indigo-50 text-indigo-500 border border-indigo-100">PO</span>
          )}
        </div>
        {box.line_count > 0 ? (
          <div className="flex items-center gap-1.5">
            <span className="text-[9px] font-bold text-gray-400">{box.line_count} item{box.line_count !== 1 ? 's' : ''}</span>
            {box.total_expected > 0 && (
              <>
          ...
          <p className="text-[9px] font-medium text-gray-300 italic">No PO linked</p>
        )}
      </div>
      ...
      <button
        ...
        className={`flex-shrink-0 p-1 rounded-lg transition-colors ${
          isActive ? 'hover:bg-indigo-100 text-indigo-400' : 'hover:bg-gray-100 text-gray-300'
        }`}
        title="Open details"
      >
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-indigo-50 border-b border-indigo-100">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[9px] font-black uppercase tracking-widest text-indigo-600 leading-none">
              Line Items
            </p>
            <p className="text-[10px] font-black font-mono text-indigo-800 mt-0.5 truncate">
          ...
          <span className={`ml-1 text-[9px] font-black rounded-full px-1.5 py-0.5 ${lines.length > 0 ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
            {lines.length}
          </span>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        {isLoading && lines.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 gap-2 opacity-40">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest">Fetching lines…</p>
          </div>
        ) : lines.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 gap-2 opacity-30">
            ...
            <p className="text-[8px] text-gray-400 text-center px-4">
              ...
              return (
                <div key={line.id} className="px-3 py-2.5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-start gap-2">
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] font-bold text-gray-800 leading-snug truncate">
                        {line.item_name || line.sku || `Line #${line.id}`}
                      </p>
                      {line.sku && line.item_name && (
                        <p className="text-[8px] font-mono text-gray-400 mt-0.5">{line.sku}</p>
                      )}
                      <div className="flex flex-wrap items-center gap-1 mt-1">
                        <span className={`text-[7px] font-black uppercase tracking-wider rounded px-1.5 py-0.5 ${badge}`}>
                          {line.workflow_status.replace(/_/g, ' ')}
                        </span>
                        {line.needs_test && (
                          <span className="text-[7px] font-black uppercase tracking-wider rounded px-1.5 py-0.5 bg-orange-100 text-orange-600">
                            Test
                          </span>
                        )}
                        {line.assigned_tech_name && (
                          <span className="text-[8px] font-medium text-gray-400 truncate">
                            → {line.assigned_tech_name}
                          </span>
                        )}
                      </div>
                      {line.zoho_purchaseorder_id && (
                        <p className="text-[8px] font-mono text-indigo-400 mt-0.5 truncate">
                    ...
                    <div className="flex-shrink-0 text-right">
                      <span className="text-[12px] font-black tabular-nums text-gray-700">
                        ...
                        <span className="text-gray-400 text-[11px]">{line.quantity_expected ?? '?'}</span>
                      </span>
                  ...
                  {line.notes && (
                    <p className="mt-1.5 text-[9px] font-medium text-gray-400 italic truncate">
                      {line.notes}
                    </p>
                  )}
                </div>
    ...
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[9px] font-black rounded-full px-1.5 py-0.5 tabular-nums ${total > 0 ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
            {total}
          </span>
        ...
        <button
          type="button"
          onClick={() => refetch()}
          className="text-[8px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-600 transition-colors px-1.5 py-0.5 rounded hover:bg-gray-100"
        >
          ↻
        </button>
      </div>
      ...
      {/* Filter tabs */}
      <div className="flex gap-1 px-3 py-1.5 border-b border-gray-50 flex-shrink-0">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
            onClick={() => { setStatusFilter(tab.id); setActiveBoxId(null); }}
