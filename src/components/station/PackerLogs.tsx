...
export default function PackerLogs({ 
    history: initialHistory, 
    isLoading: isInitialLoading, 
    packerId
}: PackerLogsProps) {
    const [history, setHistory] = useState<PackerLog[]>(initialHistory);
    ...
    const loadMore = useCallback(async () => {
        if (isLoadingMore || !hasMore || !packerId) return;
        
        setIsLoadingMore(true);
        try {
            const nextOffset = offset + limit;
            const res = await fetch(`/api/packing-logs?packerId=${packerId}&limit=${limit}&offset=${nextOffset}`);
    ...
    }, [offset, hasMore, isLoadingMore, packerId]);
    ...
    const groupedHistory: { [key: string]: PackerLog[] } = {};
    history.forEach(log => {
