...
export interface FbaShippedTableProps {
  packedBy?: number;
  testedBy?: number;
  refreshTrigger?: number;
}

export function FbaShippedTable({
  packedBy,
  testedBy,
  refreshTrigger = 0,
}: FbaShippedTableProps = {}) {
  ...
  const weekOffset = Math.max(0, Number.parseInt(searchParams.get('fbaShippedWeekOffset') || '0', 10) || 0);
  const weekRange = getWeekRangeForOffset(weekOffset);
  const formatDate = (dateStr: string) => formatDateWithOrdinal(dateStr);

  const queryKey = ['dashboard-table', 'shipped-fba', { weekStart: weekRange.startStr, weekEnd: weekRange.endStr, packedBy, testedBy }] as const;
  ...
  const setWeekOffsetInUrl = (nextOffset: number) => {
    ...
    if (nextOffset <= 0) params.delete('fbaShippedWeekOffset');
    else params.set('fbaShippedWeekOffset', String(nextOffset));
    const nextSearch = params.toString();
    const nextPath = pathname || '/fba';
  ...
  const fbaRecords = dedupedRecords.filter((record) => isFbaOrder(record.order_id, record.account_source));
  const normalizedSearch = search.trim().toLowerCase();
  const records = normalizedSearch
    ? fbaRecords.filter((record) => {
        const haystack = [
                    ...
                    return (
                      <div key={date} className="flex flex-col">
                        ...
                        {sortedRecords.map((record, index) => {
                          ...
                          const fnskuCandidate = String(record.scan_ref || record.shipping_tracking_number || '').trim().toUpperCase();
                          const fnskuValue = fnskuCandidate.startsWith('X00') ? fnskuCandidate : '';
                              ...
                              <div className="flex flex-col min-w-0">
                                ...
                                <div className="mt-0.5 flex items-center gap-2">
                                  <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest truncate min-w-0 flex-1 pl-4">
                                    ...
                                    {' • '}
                                    {fnskuValue ? `FNSKU ${getLast4(fnskuValue)}` : 'No FNSKU'}
                                    {' • '}
                                    {techDisplay}
                                    {' • '}
                                    {packerDisplay}
