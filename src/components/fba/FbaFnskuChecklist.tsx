...
import {
  useFnskuChecklistData,
  readStoredPlanSelection,
  writeStoredPlanSelection,
  isPlanChecklistVisibleItem,
  type PlanItem,
  type DraftItem,
} from '@/hooks/fba/useFnskuChecklistData';
import {
...
export function FbaFnskuChecklist({
  ...
  // Data hook — all fetching, mutations, and derived state
  const {
    isDraftMode, isTodayMode, isViewMode,
    draftItems, draftLoading, newDraftItems, alreadyInPlanCount, notFoundCount,
    planItems, planLoading, planRef, planDueDate, planAmazonShipmentId, planTrackingNumbers,
    activePlanId, visiblePlanItems, filteredPlanItems,
    creating, createError, created, addedCount, skippedCount,
    cyclingItemId, expandedNoteId, noteText, setExpandedNoteId, undoToast,
    adjustPlanQty, adjustDraftQty, removeDraftItem,
    handleAddToTodayPlan, handleMoveToPrint, handleUndoMoveToPrint,
    handleCycleStatus, handleOpenNote, handleNoteChange,
    loadPlanItems, loadTodayPlan,
  } = useFnskuChecklistData({ fnskus, planId, statusFilter, onCreated, onClear });
