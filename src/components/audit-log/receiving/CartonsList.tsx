...
export function CartonsList({ cartons }: { cartons: Carton[] }) {
  if (cartons.length === 0) {
    return <div className="py-8 text-center text-sm text-text-faint">No packages matched yet.</div>;
  }
  return (
                  ...
                  <a key={p.id} href={p.url} target="_blank" rel="noreferrer" className="group">
                    <HoverTooltip label={`${p.taken_by_name ?? 'Unknown'} · ${fmtTime(p.taken_at)}`} asChild focusable={false}>
                      ...
                      <img
                        ...
                        className="h-16 w-16 rounded-md border border-border-soft object-cover transition group-hover:ring-2 group-hover:ring-emerald-300"
                      />
                    </HoverTooltip>
                  </a>
                ))}
