...
/**
 * /admin/inventory/cycle-counts/[id]
 *
...
async function submitCountAction(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/admin/inventory/cycle-counts/${campaignId}`);
}
...
export default async function CycleCountDetailPage({
  ...
  if (!Number.isFinite(id) || id <= 0) {
    redirect('/admin/inventory/cycle-counts');
  }
        ...
        <nav className="flex flex-wrap gap-2 text-xs">
          {(['all', 'pending', 'counted', 'pending_review', 'approved', 'rejected'] as const).map((s) => {
            ...
            return (
              <Link
                key={s}
                href={`/admin/inventory/cycle-counts/${id}?status=${s}`}
