...
function OrdersManagement() {
    ...
    const assignOrderMutation = useMutation({
        mutationFn: async ({ orderId, testerId, packerId, shipByDate, outOfStock }: { orderId: number; testerId?: number | null; packerId?: number | null; shipByDate?: string; outOfStock?: string }) => {
            const res = await fetch('/api/orders/assign', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ orderId, testerId, packerId, shipByDate, outOfStock }),
            });
                    ...
                    orders.map((order) => (
                        <div key={order.id} className="p-5 rounded-3xl bg-white border border-gray-200 transition-all hover:shadow-sm">
                            <div className="flex items-start justify-between mb-3">
                                ...
                                {order.out_of_stock && (
                                    <div className="flex flex-col gap-2">
                                        <span className="px-3 py-1 rounded-xl text-[8px] font-black uppercase tracking-widest bg-amber-100 text-amber-700">
                                            {order.out_of_stock}
                                        </span>
                                    </div>
                                )}
                            ...
                            <div className="flex items-center gap-2 text-xs text-gray-500">
                                {order.tester_id && <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded">Tester: {order.tester_id}</span>}
                                {order.packer_id && <span className="px-2 py-1 bg-green-100 text-green-700 rounded">Packer: {order.packer_id}</span>}
                                {!order.tester_id && !order.packer_id && <span className="text-gray-400">Unassigned</span>}
                            </div>
                        </div>
                    ))
                )}
