...
/**
 * /admin/inventory-v2/bulk-allocate
 *
...
async function allocateOne(formData: FormData): Promise<void> {
  ...
  revalidatePath('/admin/inventory-v2/bulk-allocate');
}
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Bulk allocate</h1>
          ...
          <header className="flex items-center justify-between border-b border-gray-100 px-6 py-3">
            ...
            {total > PAGE_SIZE ? (
              <nav className="flex items-center gap-2 text-sm">
                {page > 0 ? (
                  <Link href={`/admin/inventory-v2/bulk-allocate?page=${page - 1}`} className="rounded border border-gray-300 px-3 py-1 hover:bg-gray-50">
                    ← prev
                  </Link>
                ) : null}
                {page + 1 < totalPages ? (
