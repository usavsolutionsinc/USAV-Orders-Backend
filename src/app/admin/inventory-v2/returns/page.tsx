...
async function intakeAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      redirect(`/admin/inventory-v2/returns?error=${result.status === 404 ? 'not_found' : 'failed'}${detail}`);
    }
  } catch (err) {
    console.error('[returns.intake] failed:', err);
    redirect('/admin/inventory-v2/returns?error=failed');
  }

  revalidatePath('/admin/inventory-v2/returns');
  redirect('/admin/inventory-v2/returns?ok=1');
}
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Returns intake</h1>
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td className="px-4 py-2 font-mono text-xs">
                          {r.serial_unit_id ? (
                            <Link href={`/admin/inventory-v2/units/${r.serial_unit_id}`} className="text-blue-600 hover:underline">
                              #{r.serial_unit_id}
                            </Link>
                          ) : '—'}
                        </td>
                        <td className="px-4 py-2 font-mono text-xs">
                          {r.sku ? (
                            <Link href={`/admin/inventory-v2/sku/${encodeURIComponent(r.sku)}`} className="text-blue-600 hover:underline">
                              {r.sku}
                            </Link>
                          ) : '—'}
                        </td>
                        <td className="px-4 py-2 text-xs text-gray-600">{r.prev_status ?? '—'}</td>
                        <td className="px-4 py-2 font-mono text-[11px] text-gray-600">{r.scan_token ?? '—'}</td>
