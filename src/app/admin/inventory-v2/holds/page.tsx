...
async function holdAction(formData: FormData): Promise<void> {
  ...
  if (!isInventoryV2Returns()) {
    redirect('/admin/inventory-v2/holds?error=flag_off');
  }
  const refRaw = String(formData.get('ref') ?? '').trim();
  const reason = String(formData.get('reason') ?? '').trim();
  if (!refRaw || !reason) {
    redirect('/admin/inventory-v2/holds?error=missing_input');
  }
  ...
  if (!Number.isFinite(serialUnitId) || serialUnitId <= 0) {
    redirect('/admin/inventory-v2/holds?error=not_found');
  }
  ...
  revalidatePath('/admin/inventory-v2/holds');
}
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold text-gray-900">Holds</h1>
                  ...
                  {held.map((h) => (
                    <tr key={h.id}>
                      <td className="px-4 py-2 font-mono text-xs">
                        <Link href={`/admin/inventory-v2/units/${h.id}`} className="text-blue-600 hover:underline">
