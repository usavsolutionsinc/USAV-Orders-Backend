      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="space-y-1">
          ...
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <h1 className="text-2xl font-semibold text-gray-900">Throughput</h1>
            <nav className="flex items-center gap-1 text-xs">
              {(['24h', '72h', '7d'] as const).map((r) => (
                <Link
                  key={r}
                  href={`/admin/inventory-v2/throughput?range=${r}`}
              ...
              <table className="text-xs">
                <thead>
                  <tr>
                    ...
                    {hourlyBuckets.map((iso) => (
                      <th key={iso} className="px-1 py-1 text-center font-normal text-[10px] text-gray-400">
                ...
                <tbody>
                  {hourlyStations.map((station) => (
                    <tr key={station}>
                      <td className="px-2 py-1 font-mono text-[11px] text-gray-700">{station}</td>
                      {hourlyBuckets.map((iso) => {
