...
/**
 * Per-provider integration card. Renders the status pill, the display label,
 * and a tiny actions row (Connect / Update / Disconnect). The actual
 * credential entry happens in a modal-style sheet because each provider's
 * payload shape is different — we keep that detail in this component so
 * adding a provider is one-file work.
 *
 * MVP: textarea-of-JSON for the payload. The roadmap calls for per-provider
 * structured forms (OAuth-button for eBay/Zoho/Square, scope toggles, etc.)
 * but this is good enough to be functional from day one.
 */
