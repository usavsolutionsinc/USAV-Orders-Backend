...
export async function GET(
  ...
  try {
    ...
    // Tenant ownership pre-flight. The trail queries below key on the order id
    // alone; without this an authed caller in org A could read org B's audit +
    // inventory_events via a guessed order id. 404 (not 403) so we don't reveal
    // that an order with that id exists in another tenant.
    const owner = await pool.query<{ organization_id: string | null; shipment_id: number | null }>(
      `SELECT organization_id, shipment_id FROM orders WHERE id = $1`,
      [id],
    );
    if (owner.rows.length === 0 || owner.rows[0].organization_id !== gate.ctx.organizationId) {
    ...
    const [result, alloc, stationEvents] = await Promise.all([
      ...
      pool.query<{ serial_unit_id: number }>(
