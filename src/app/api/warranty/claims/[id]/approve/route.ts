...
import { approveClaim } from '@/lib/warranty/mutations';
...
/**
 * POST /api/warranty/claims/[id]/approve — SUBMITTED → APPROVED. Gated by WARRANTY_LOGGER.
 */
export const POST = withAuth(async (request, ctx) => {
  ...
  return idempotentJson({
    ...
    route: 'POST /api/warranty/claims/[id]/approve',
    bodyKey,
    produce: async () => {
      const result = await approveClaim(id, ctx.staffId ?? null, ctx.organizationId);
