  ...
  try {
    ...
    const orderRes = await pool.query(
      `SELECT id, status, customer_name, pickup_date::text AS pickup_date, notes
       FROM local_pickup_orders WHERE id = $1`,
      [orderId],
    );
    ...
    const itemsRes = await pool.query(
      `SELECT id, sku, product_title, quantity, condition_grade, parts_status,
       ...
       FROM local_pickup_order_items WHERE order_id = $1 ORDER BY id ASC`,
      [orderId],
    );
    ...
    try {
      ...
      const upd = await client.query(
        `UPDATE local_pickup_orders
         ...
         RETURNING *`,
        [orderId, zohoPo.purchaseorder_id, poNumber, receivingId],
      );
      if (upd.rows.length === 0) {
        // Raced (already finalized elsewhere). Roll back; the Zoho PO is a
        // harmless duplicate-protected by the unique zoho_po_id index.
      ...
      await client.query(
        `UPDATE local_pickup_order_items SET receiving_id = $2, updated_at = NOW() WHERE order_id = $1`,
        [orderId, receivingId],
      );
      ...
      for (const [sku, zid] of zohoItemBySku) {
        await client.query(
          `UPDATE local_pickup_order_items SET zoho_item_id = $3, updated_at = NOW()
           WHERE order_id = $1 AND BTRIM(sku) = $2`,
          [orderId, sku, zid],
        );
      }
