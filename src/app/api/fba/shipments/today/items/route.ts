...
export const POST = withAuth(async (request: NextRequest, ctx) => {
  try {
    ...
    const { shipmentId, shipmentRef, added, merged, moved, skipped } = await withTenantTransaction(
      ctx.organizationId,
      async (client) => {
        ...
        if (todayShip.rows.length === 0) {
          ...
          const newShip = await client.query(`
            ...
            VALUES ($1, CURRENT_DATE, 'PLANNED', $2)
            RETURNING id, shipment_ref
          `, [shipmentRef, ctx.organizationId]);
        ...
        for (const item of items) {
          ...
          const othersRes = await client.query<{ id: number; expected_qty: number }>(
            `SELECT fsi.id, fsi.expected_qty
             ...
             JOIN fba_shipments fs ON fs.id = fsi.shipment_id AND fs.organization_id = fsi.organization_id
             WHERE fsi.fnsku = $1
               AND fs.id <> $2
            ...
            [fnsku, shipmentId, ctx.organizationId]
          );
          ...
          if (existingFnskus.has(fnsku)) {
            if (otherIds.length > 0) {
              ...
              await client.query(
                `DELETE FROM fba_shipment_items WHERE id = ANY($1::int[]) AND organization_id = $2`,
                [otherIds, ctx.organizationId]
              );
            }
            const bumpRes = await client.query<{
              ...
              `UPDATE fba_shipment_items fsi
               SET expected_qty = $1::int,
                   updated_at = NOW()
               WHERE fsi.shipment_id = $2 AND fsi.fnsku = $3 AND fsi.organization_id = $4
               RETURNING
                 fsi.id,
                 fsi.fnsku,
                 fsi.expected_qty,
                 COALESCE(
                   ...
                   (SELECT ff.product_title FROM fba_fnskus ff WHERE ff.fnsku = fsi.fnsku AND ff.organization_id = fsi.organization_id),
                   fsi.fnsku
                 ) AS display_title`,
              [qty, shipmentId, fnsku, ctx.organizationId]
            );
            ...
            const moveRes = await client.query<{
              ...
              `UPDATE fba_shipment_items fsi
               SET shipment_id = $1,
                   expected_qty = $2::int,
                   updated_at = NOW()
               WHERE fsi.id = $3 AND fsi.organization_id = $4
               RETURNING
                 fsi.id,
                 fsi.fnsku,
                 fsi.expected_qty,
              ...
              [shipmentId, totalQty, keepId, ctx.organizationId]
            );
            await client.query(
          ...
          const itemRes = await client.query(
            `INSERT INTO fba_shipment_items
               (shipment_id, fnsku, expected_qty, product_title, asin, sku, status, organization_id)
             VALUES ($1, $2, $3, $4, $5, $6, 'PLANNED', $7)
             RETURNING id`,
            [shipmentId, resolvedFnsku, qty, catalogTitle, catalogAsin, catalogSku, ctx.organizationId]
          );
        ...
        return { shipmentId, shipmentRef, added, merged, moved, skipped };
      }
    );
