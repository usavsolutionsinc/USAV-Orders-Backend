...
import {
  getMediaSavedView,
  updateMediaSavedView,
  deleteMediaSavedView,
} from '@/lib/photos/saved-views-queries';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const gate = await requireRoutePerm(req, 'photos.view');
    ...
    if (typeof raw.isShared === 'boolean') {
      // Only a manager may share a view org-wide; un-sharing your own is fine.
      if (raw.isShared === true && !gate.ctx.permissions.has('photos.manage')) {
        return NextResponse.json(
          { success: false, error: 'Sharing a view org-wide requires manage permission' },
          { status: 403 },
        );
      }
      patch.isShared = raw.isShared;
    }
    if (Number.isFinite(Number(raw.sortOrder))) patch.sortOrder = Number(raw.sortOrder);

    const before = await getMediaSavedView(id, gate.ctx.organizationId);
    if (!before || before.staff_id !== gate.ctx.staffId) {
      return NextResponse.json({ success: false, error: 'Not found' }, { status: 404 });
    }

    const updated = await updateMediaSavedView(id, gate.ctx.organizationId, gate.ctx.staffId, patch);
    ...
    await recordAudit(pool, gate.ctx, req, {
      ...
      action: AUDIT_ACTION.MEDIA_SAVED_VIEW_UPDATE,
    ...
    console.error('[PATCH /api/photos/saved-views/[id]] error:', error);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await deleteMediaSavedView(id, gate.ctx.organizationId, gate.ctx.staffId);

    await recordAudit(pool, gate.ctx, req, {
      source: 'media-saved-views-api',
      action: AUDIT_ACTION.MEDIA_SAVED_VIEW_DELETE,
      entityType: AUDIT_ENTITY.MEDIA_SAVED_VIEW,
      entityId: id,
      before: { ...before },
    });
    ...
    console.error('[DELETE /api/photos/saved-views/[id]] error:', error);
    return NextResponse.json({ success: false, error: message }, { status: 500 });
  }
}
