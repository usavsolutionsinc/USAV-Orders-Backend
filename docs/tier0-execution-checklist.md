...
- [ ] **[you]** Create non-BYPASSRLS `app_tenant` role on Neon (template `2026-06-21_app_tenant_role.sql.template`); verify `rolbypassrls=f`.
- [ ] **[you]** Run the `cross-org-isolation.test.ts` canary (`TENANT_APP_DATABASE_URL=<dsn>`) — proves isolation on a scratch table.
- [ ] **[you]** Set `TENANT_APP_DATABASE_URL` = app_tenant DSN in prod (keep `DATABASE_URL` = owner); redeploy; `npm run tenancy:guard:check`.
...
- [ ] Per-table E2 enforce migrations as each table's routes go all-`low` (then `reason_codes`, `warranty`, `fba`, …).
...
- [~] **FORCE prerequisite (schema): WRITTEN, UNAPPLIED.** `sku_catalog`(sku) → composite UNIQUE in `src/lib/migrations/2026-06-14_sku_catalog_composite_unique.sql` (clean — FKs hang off `id`); `fba_fnskus`(fnsku) PK → composite in `2026-06-14_fba_fnskus_composite_pk.sql` (drops + recreates the 4 child FKs as composite; PG15 `ON DELETE SET NULL (fnsku)` for the 2 NOT-NULL-org children; includes a pre-flight data check). **Both must land WITH their `ON CONFLICT` code flips** (listed in each file's header) — applying standalone breaks the upserts. Apply after each table's routes are all-`low`.
