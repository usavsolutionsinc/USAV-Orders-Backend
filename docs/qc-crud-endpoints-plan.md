...
>
> **Migration status:** `2026-06-06_qc_template_lifecycle.sql` is **applied**. New columns live now:
...
**Deferred:** value-kind skip in bulk-pass (numeric steps shouldn't blanket-pass), failure-mode migration + auto-tag-on-fail.
...
- [ ] `withAuth({ permission })` or `requireRoutePerm` guard present
- [ ] `parseBody(<zod>)` → 400 on invalid; no raw `body.x` reads
- [ ] Idempotency key honored on POST creates (catalog authoring route)
- [ ] `recordAudit` with canonical `AUDIT_ACTION` + before/after
- [ ] `passMin <= passMax`; `value_enum` only when `value_kind='ENUM'`
- [ ] Execution reads stay `status='published'`; authoring reads show all
- [ ] api-route-reviewer + permission-registry-guard pass (no new perms expected)
