...
**STATUS: DONE (100%). Status verified 2026-06-28.**
Phase 1 + Phase 2 (option a) shipped; capabilities registry expanded. Plan complete — safe to archive.

**Status:** plan + Phase 1 + Phase 2 implemented (2026-06-06; verified DONE 2026-06-28)
...
- `workspace-capabilities.ts` shipped (`ReceivingWorkspaceVariant`, `WorkspaceCapabilities`, `WORKSPACE_CAPABILITIES`, `workspaceCapabilities()`); registry expanded with `notes` / `openInUnbox` / `matching` / `poItems`.
- `variant` threaded `RightPane`(:200) → `ReceivingLineWorkspace`(:134) → `LineEditPanel`.
- All Phase-1 gating (label / receive-bar / photos / claim / serial-scan / save-bar) confirmed live.
- Phase-2 option (a) confirmed: `activeRowSlot` returns `null` when `!caps.serialScan` (`LinePoItemsSection`:111).
- STALE FIX: any "Phase 2 serial/condition gating remains" note (README/seed) is FALSE — it is done.

## Remaining work — handoff (2026-06-28)

None — plan complete; safe to archive out of `docs/partial/`.

- [DEFERRED-BY-DESIGN] The illustrative future `qc` / `tech` variant columns in §5 are intentionally not built; they are documentation of registry extensibility, not pending work.
