...
> **STATUS: DONE (95%) — code-complete; only Amazon-side approval remains. Status verified 2026-06-28.**
> All 4 phases shipped (`src/lib/amazon/*`, 6 routes, cron, migrations 2026-06-14b/c, `integrations.amazon`
> perm, Connections UI). Sole remaining item is owner-gated Amazon Appstore publish + restricted PII role.
...
- **Phase 3 — Item-scoped auto-import. [x] DONE 2026-06-28.** `matchesTrackedItem` predicate (SellerSKU ∈
  catalog / tracked FBA ASINs) + the tracked-items-only default toggle. ✅ only orders for tracked SKUs/FBA
  items import. (Item-scope filter verified present.)
- **Phase 4 — PII + hardening. [x] DONE 2026-06-28.** RDT inline for MFN ship-orders + encrypted address
  storage/retention; 365-day re-auth reminder; chip convergence on `sales_orders.channel`; optional v2026-01-01
  mapper. (RDT PII path verified present.)
...
- **Phase 3 BUILT [x] (verified 2026-06-28):** item-scoped auto-import (SellerSKU/ASIN filter).
- **Phase 4 BUILT [x] (verified 2026-06-28):** RDT PII for MFN ship-orders. (Prior README/seed note "Phase 3
  auto-import + Phase 4 PII remain" was STALE — both shipped.)
...
- Audit confirms all 4 phases are code-complete: `src/lib/amazon/*`, 6 routes, cron in `vercel.json`,
  migrations 2026-06-14b/c, `integrations.amazon` permission, Connections UI.
- Orders land in `orders` + `customers` (not `sales_orders`); Phase 3 item-scope filter and Phase 4 RDT PII
  both verified present.
- Corrected stale README/seed claim that "Phase 3 auto-import + Phase 4 PII remain" — both shipped.

## Remaining work — handoff (2026-06-28)
- **[OWNER-GATED]** Publish the Amazon SP Appstore app and obtain the restricted PII (Direct-to-Consumer
  Delivery) role for true multi-tenant OAuth and live PII. The self-authorization paste path
  (`/api/amazon/connect`) works today as the bootstrap; no code change needed — this is an Amazon-side approval.
- **[MIGRATION-VERIFY]** Confirm migrations 2026-06-14b/c are applied in prod (non-gated, additive).
- **[DEFERRED-BY-DESIGN]** Optional v0 → 2026-01-01 Orders API mapper cutover (deferred; `raw` JSONB already
  stored so cutover stays a mapper change).

Plan is COMPLETE/archivable — safe to move out of `docs/partial/` once the migration-verify is confirmed.
