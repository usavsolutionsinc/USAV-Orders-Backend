...
**STATUS: MOSTLY DONE (80%). Status verified 2026-06-29 (db ledger 0 pending).**
Phases 1/4/5 fully shipped; Phase 3 platform+type reads done and order-channel display wiring is in progress (6 surfaces wired this session). `2026-06-14f` is now APPLIED & live. Remaining: finish order-surface label wiring, run the backfill (dry-run → `--apply`), and the deferred Phase 6 text-column drop.
...
**Migration APPLIED ✅ (verified 2026-06-29, db ledger 0 pending):** `2026-06-14f_catalog_type_fk_accounts_seed.sql` (adds `receiving.type_id` + `orders.type_id` FKs; seeds `platform_accounts`). Columns + seed are live. Residual: run the backfill to populate `type_id`. All dual-write is column-existence-guarded, so the app was safe before/after.

**Phase scorecard:** Phase 1 ✅ · Phase 2 ✅ (built; migration unapplied) · Phase 3 ✅ (platform + type; order-channel *resolver* ✅, surface wiring 🟡 — **6 surfaces wired 2026-06-28**, remainder open) · Phase 4 ✅ (platforms + types + **accounts**) · RLS ✅ · Phase 5 ✅ (account + workflow-node bindings) · Phase 6 ⬜ (deferred by design — gated).
...
### A. Run Phase 2 backfill (operational, not code)
1. ~~**Apply `2026-06-14f`.**~~ **DONE** — APPLIED & live (verified 2026-06-29, db ledger 0 pending). `receiving.type_id` + `orders.type_id` columns and the `platform_accounts` seed exist.
...
- **Recreated `scripts/backfill-catalog-type-id.mjs`** (it had gone missing from the tree): dry-run default, `--apply`, `--org` scoping, migration-gated to a no-op when the `type_id` column is absent; maps `receiving.type_id` from the effective `intake_type` via `receivingTypeSlug`.
- **Started Phase 3 order-channel display wiring** — `useOrderChannelLabel` (previously 0 call-sites) is now wired into 6 surfaces: `OrdersQueueTableRow`, `ShippedRecordRow`, `PackerRecordRow`, `TechRecordRow`, `AddTrackingPopover`, `useUpNextController`.
- **`OrderGroupSummary` now wired** — converted from the static `getOrderPlatformLabel` to `useOrderChannelLabel()` (hook called at component top level, labeler used in render; rules-of-hooks respected). With this, the order-channel display surfaces are **effectively complete** — the only remaining call-sites of the static helper are the intentionally-static `PlatformBadge` (`src/design-system/components/PlatformBadge.tsx`) and `work-order-assignment-shared.ts` (a module-scope map callback where a hook is unsafe). Text/casing is byte-identical via the org catalog text-cache + built-in fallback until a catalog override exists.
- Confirmed Phases 1/4/5 fully shipped and Phase 3 platform+type reads done (audit-verified against code).
- Migration status verified 2026-06-29 (db ledger 0 pending): `2026-06-14f_catalog_type_fk_accounts_seed.sql` is APPLIED & live — `receiving.type_id` + `orders.type_id` columns + `platform_accounts` seed exist. Residual: run `scripts/backfill-catalog-type-id.mjs` (dry-run → `--apply`); Phase 6 text-column drops stay deferred.

## Remaining work — handoff (2026-06-28)

- **[CODE]** ~~Finish the remaining order-surface label wiring. `OrderGroupSummary` is safe to convert to `useOrderChannelLabel`.~~ **DONE** — `OrderGroupSummary` converted (2026-06-28). Order-channel display surfaces effectively complete. `PlatformBadge` and `work-order-assignment-shared` are intentionally left on the static helper (module-scope / map-callback contexts where a hook is unsafe).
- **[MIGRATION-DEPLOY-COUPLED]** ~~Apply `2026-06-14f_catalog_type_fk_accounts_seed.sql` deliberately.~~ **APPLIED ✅** (verified 2026-06-29, db ledger 0 pending) — `receiving.type_id` + `orders.type_id` columns + `platform_accounts` seed are live. Residual: run the backfill dry-run → `--apply` to populate `type_id`.
- **[DEFERRED-BY-DESIGN]** Phase 6 — drop the `source_platform`/`intake_type` text columns + CHECK constraints. High-risk: still a large live read footprint (incl. `'fba'` checks). Hold until readers move to the resolvers.
- **[CODE]** Hygiene — commit the catalog working-tree changes.
