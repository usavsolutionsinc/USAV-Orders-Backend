...
**FINAL STATUS: CODE-COMPLETE, INFRA-PENDING (55%).** App side (proxy, agent code,
README, Admin UI) is fully built and verified; the only remaining work is owner-gated
office infrastructure + Vercel env. Status verified 2026-06-28.
...
- Verified every symbol in the "App side — DONE" matrix exists: `resolveNasReceivingUpstream` / `resolveNasReceivingWriteUpstream`, the `/api/nas-target` route, `syncAgentRootsFromSettings`, `normalizePhotoDisplayUrl`, and the Zendesk archive path.
- Verified the full NAS Media Agent (`deploy/nas-media-agent/server.mjs`: `/health`, `PUT`/`DELETE /file`, `PUT`/`GET /roots`, `POST /archive`) + `README.md` are in-repo.
- Verified Admin UI is built: `StationNasFoldersTab`, `NasWorkflowsPanel`.
- Conclusion: nothing AI-actionable remains in-repo; all remaining work is office infra + Vercel env.

---

## Remaining work — handoff (2026-06-28)

- [OWNER-GATED] Deploy the full agent on the office Mac (Phase 1.1) and retire the slim archive-only `:8787` agent (Phase 1.4) — do not run two agents on one port.
- [OWNER-GATED] Wire Caddy `handle_path /_agent/* → 127.0.0.1:8787` (Phase 1.2), then add LaunchAgents for the agent + Caddy and a `cloudflared` system service so all three survive reboot (Phase 1.3).
- [OWNER-GATED] Set Vercel Production `NAS_RW_URL`, `NAS_AGENT_URL`, `NAS_AGENT_TOKEN` (Phase 2) and redeploy production.
- [OWNER-GATED] Run the Phase 5 e2e probe: PUT 201 through the tunnel; mobile capture → `POST /api/receiving-photos` 200 → photo visible → gallery delete 204. Close this plan once Phase 5 passes.
- [CODE] None — app side is code-complete.
