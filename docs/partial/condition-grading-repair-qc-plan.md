...
> **FINAL STATUS: DONE.** Status verified 2026-06-29 — all migrations APPLIED & live (db ledger 0 pending). Backend + tech detail-pane + admin analytics complete end-to-end; only live eBay reverse-push remains (deferred out of v1 by design).
...
**Migration** — `src/lib/migrations/2026-06-06_qc_template_lifecycle.sql` (additive, `IF NOT EXISTS`, `status` defaults `'published'` ⇒ zero behavior change). **APPLIED & live** — verified 2026-06-29 (db ledger 0 pending).
...
**Backfill** — `scripts/backfill-unit-quality.ts` (`tsx … [--dry]`), batched/sequential; not auto-run (GET self-heals per unit). **DONE 2026-06-28** — script created this session (it had been claimed shipped but was missing); reuses `recomputeUnitQuality`, dry-run default + `--apply` + `--org`, org-scoped and idempotent.
...
- Created `scripts/backfill-unit-quality.ts` — the §16 backfill script the doc claimed shipped but was actually missing. Reuses `recomputeUnitQuality`, dry-run by default with `--apply` to commit and `--org` to scope, org-scoped and idempotent.
- Reconciled doc status to VERIFIED reality: all migrations, `src/lib/quality/*`, queries, every route, grade advisory, `GRADE_TO_EBAY_CONDITION`, `UnitQualityPanel`, and `QualityDashboardTab` confirmed shipped.
- Migration status verified 2026-06-29 (db ledger 0 pending): `2026-06-06_qc_template_lifecycle.sql` and `2026-06-07_failure_modes.sql` are APPLIED & live (with `_unit_repairs.sql` + `_unit_quality_scores.sql`) — full phase migration set now applied.

## Remaining work — handoff (2026-06-29)

- **[MIGRATION-VERIFY] — VERIFIED APPLIED ✅ (2026-06-29, db ledger 0 pending).** `src/lib/migrations/2026-06-06_qc_template_lifecycle.sql` and `2026-06-07_failure_modes.sql` are APPLIED & live; the other phase migrations (`_unit_repairs.sql`, `_unit_quality_scores.sql`) were already confirmed applied. Full phase migration set is now live.
- **[OWNER-GATED]** Optionally run the new `scripts/backfill-unit-quality.ts` dry-run, then `--apply`. Non-urgent — the `GET …/quality` route self-heals each unit on read.
- **[DEFERRED-BY-DESIGN]** Live eBay reverse-push of condition/score — explicitly out of v1 scope; compute + display only.
