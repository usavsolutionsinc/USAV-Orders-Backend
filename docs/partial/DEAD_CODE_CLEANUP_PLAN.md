...
> **LIVING TRACKER — hard deliverables DONE (78%).** Status verified 2026-06-28.
> Tooling, CI ratchet, Phase 2 route deletions, and Phase 4.1 flag collapse are complete; Phase 3 knip waves continue as a living backlog.
...
### 4.1 Inventory V2 dual implementations — DONE 2026-06-28

> **[x] DONE (collapsed 2026-06-14):** the `INVENTORY_V2` dual-path flag was collapsed and the V1 `else` branches removed; the engine now runs unconditionally. Verified 2026-06-28.
...
**Current focus**: Phase 3 ongoing (next: fba fragments, manuals legacy pieces, labels if safe). Phase 4 (V2 flags, shipped table) pending deeper review.

**Phase 7 — prevention gate now ACTIVE (Wave 2)**: knip runs in CI as a baseline ratchet.
- Gate: `npm run knip` (`scripts/knip-gate.mjs`) compares the current knip findings against the committed `knip-baseline.json` and fails CI **only on findings not already in the baseline** (NEW dead code). The existing backlog is absorbed by the baseline, so the gate is green on the current tree.
- Wired into `.github/workflows/ci.yml` (`ci` job → "Dead-code gate (knip baseline)" step), no secrets, runs on fork PRs too.
- Determinism: the gate wipes `node_modules/.cache/knip` and runs `knip --cache` (clean cold rebuild). This is required — plain `knip` is mildly non-deterministic for files touching optional deps (googleapis, zen engine). It matches CI, where `npm ci` wipes the cache each job.
- Fingerprints are line-independent (`category|file|name`), so unrelated edits don't churn the baseline and cleanup never trips the gate — only additions do.
- Refresh after intentional changes: `npm run knip:baseline` (regenerates `knip-baseline.json`; commit it). This is the escape hatch for an intentionally-kept export/type (e.g. a design-system type).
- The raw report scripts (`dead-code:knip` / `dead-code:report`) are unchanged for triage. The `knip.config.ts` `rules: { exports, types }` strictness stays deferred — the baseline gate covers regressions in the meantime.
...
Ran a conservative Phase-3 wave: verified all 26 current knip "unused files" candidates (static + dynamic + barrel + route/registry reachability + git working-tree status), **deleted 7** confirmed-dead (tsc-green after), **kept 19** (≈14 are uncommitted in-flight raw-`<button>`→primitive-sweep files; the rest have live importers / kept-barrel re-exports / live routes / test-referenced org-stamping sites). Full list in `dead-code-triage.md` → "2026-06-29 verification wave".

> **Phase 7 (knip ratchet) note:** the gate is currently red with ~83 NEW findings — **baseline drift from concurrent development** since the 2026-06-28 baseline (≈79 in-flight exports + ≈4 intentional additive color-feature exports from this session, one of which knip false-positives because the test imports it dynamically). Not caused by the deletions. **Owner action:** `npm run knip:baseline` (+ commit) once in-flight work settles to re-establish a clean ratchet floor.
