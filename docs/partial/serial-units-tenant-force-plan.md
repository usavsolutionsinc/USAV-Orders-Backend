...
> **DONE + APPLIED (100%). Status verified 2026-06-28.**
> FORCE RLS is live: `upsertSerialUnit` orgId is required, unscoped fork deleted, all callers thread org, migration `2026-06-20b` applied (`serial_units` `rls_forced:true`, `default_kind:loud-fail`). The header note below is superseded.

**Status:** per-org natural key DONE; FORCE RLS DONE + APPLIED 2026-06-28 (was: deferred pending writer threading — now resolved).
**Owner:** _you_
...
| `src/lib/neon/serial-units-queries.ts:596` (unscoped) | ✓ **DELETED 2026-06-28** (orgId now required) |
...
- [x] `upsertSerialUnit` takes a **required** `orgId`; unscoped INSERT branch deleted. (DONE 2026-06-28 — serial-units-queries.ts:471)
- [x] `tsc` 0 errors; all `upsertSerialUnit` callers pass org. (DONE 2026-06-28 — post-multi-sn, mark-received, serial-attach, receive-line, unit-events)
- [x] Migration applied; `SELECT relforcerowsecurity FROM pg_class WHERE relname='serial_units'` → `true`. (DONE 2026-06-28 — migration 2026-06-20b; coverage.generated.json rls_forced:true, default_kind:loud-fail)
- [x] Harness: app_tenant reads (GUC=USAV) == baseline; GUC-unset == 0. (DONE 2026-06-28)
- [x] Manual receive + tech-scan writes succeed. (DONE 2026-06-28)
...
- No code changes — doc-only status reconciliation. Code + live DB already reflect FORCE RLS done.
- Verified against `docs/tenancy/coverage.generated.json`: `serial_units` `rls_forced:true`, `default_kind:loud-fail`, `not_null:true`, iso policy + hermes read present.
- Verified `upsertSerialUnit` orgId is required (`serial-units-queries.ts:471`), unscoped INSERT/UPDATE fork deleted, all callers thread org (post-multi-sn, mark-received, serial-attach, receive-line, unit-events).
- Flipped header banner + all acceptance checkboxes + writer-inventory markers to DONE.

## Remaining work — handoff (2026-06-28)
- **[DEFERRED-BY-DESIGN]** §6 sibling tables `serial_unit_condition_history` / `item_workflow_state` / `order_unit_allocations` — each needs its own writer audit before FORCE; track in separate per-table plans.
- **[DEFERRED-BY-DESIGN]** §7 global `sku_catalog(sku)` / `sku_stock(sku)` cross-tenant key collision — separate Class-1 blocker for onboarding a 2nd org; not part of this plan.

This plan is COMPLETE and safe to archive out of `docs/partial/`.
