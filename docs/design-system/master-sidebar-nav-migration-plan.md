...
**P3 · Cut over the heavy stations. ✅ DONE (verified in code 2026-06-03).**
**receiving** (pathname + `?mode=`, `/receiving/unfound`), **fba**
(`resolveFbaMode`), **dashboard** (orders subview + the search overlay quirk) are
all in `MASTER_NAV_RAIL_PAGES` and gate their own pill-rows on `!masterNavEnabled`
(`ReceivingSidebarPanel:1067`, `FbaSidebar:473`, dashboard `filterControl`). The
search overlay stayed a focus param, not a 4th mode, as planned. *Still
browser-unverified — needs a scan/rail/deep-link smoke test.*

**P4 · Admin + polish. ✅ DONE (admin) 2026-06-03.**
Admin's 20+ `ADMIN_SECTION_OPTIONS` are surfaced as **grouped mode-rows in the
dropdown only** — admin is deliberately NOT in `MASTER_NAV_RAIL_PAGES` (20+ icons
don't fit the flush horizontal rail), and `AdminSidebar`'s own grouped/described
body list is kept untouched (not gated). Added `group?: string` to
`SidebarModeItem`; added the `admin` entry to `SIDEBAR_PAGE_NAV` (modes from
`ADMIN_SECTION_OPTIONS`, `?section=`, overview clears the param); `MasterNav`
permission-filters modes by `requires` (`filterPageModes`) for parity with
`AdminSidebar`; `MasterNavDropdown` renders a group header per mode-group.
- **Correction to the original P4:** `getSidebarTitle` is **kept**, not deleted —
  it's now load-bearing for `TechSidebarPanel`'s `contextNavTitle` back-nav
  (`DashboardSidebar:252`). The legacy flat `navOpen` `NavSection` + header-title
  override were already removed in P2.
- **Panels are NOT deleted:** they're the permanent body layer master-nav renders
  via `renderContext` (§4 "delete the chrome, keep the body"). Only the
  genuinely-orphaned `SalesSidebarPanel` was removed (superseded by
  `WalkInSidebarPanel` sales mode).
*Acceptance: every admin section ≤2 taps from the dropdown; tsc 0 errors;
`sidebar-navigation.test.ts` 11/11 covering all 22 admin modes.*
